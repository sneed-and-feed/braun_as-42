...
echo [2/4] Syncing assets from braun_as-42 ...
cd /d "%~dp0"
...
copy /y "ARCHITECTURE.md" "%TARGET_DIR%\" >nul
copy /y "CHANGELOG.md" "%TARGET_DIR%\" >nul
...
copy /y "start.sh" "%TARGET_DIR%\" >nul
copy /y "run.sh" "%TARGET_DIR%\" >nul
...
git add index.html .nojekyll README.md ARCHITECTURE.md CHANGELOG.md VERIFICATION_CHECKLIST.md LICENSE package.json server.js start.bat run.bat start.sh run.sh css js images -A
